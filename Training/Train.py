...
def Train():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # break
            source, target, frc = batch['jpg'].to(device), batch['sgmt'].to(device), batch['frc'].float()
        ...
        print(f'\nEPOCH: [{epoch + 1}/{num_epochs}], Mapping_Loss: {mapping_loss / count:.4f}, Confidence_Loss: {confidence_loss / count:.4f}\n')
                # mapping_loss = 0.0
                # confidence_loss = 0.0
    torch.save(branch1.state_dict(), '../Model/Weights/UNet_mod.pth')
    torch.save(branch2.state_dict(), '../Model/Weights/Confidence_mod.pth')
